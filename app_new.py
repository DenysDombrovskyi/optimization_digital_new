...
# Ви можете вибрати один із методів:

# Метод 1: Завантаження з локального файлу (як було раніше)
...
try:
    ...
    pass # Залишаємо pass, якщо цей метод закоментований
    ...
    st.error(f"Виникла помилка під час завантаження логотипу з локального файлу: {e}")

# Метод 2: Завантаження за URL-адресою
# Вставте реальну URL-адресу вашого логотипу тут.
# Для прикладу використовуємо логотип Dentsu X з вікіпедії.
...
st.image(logo_url, width=200, caption="Digital Split Optimizer Logo (через URL)") # Можете налаштувати ширину та додати підпис

# Метод 3: Використання Base64-кодування (як альтернатива, але не реалізовано тут для стислості)
# Для використання Base64 вам потрібно буде спочатку закодувати зображення в Base64 рядок,
# а потім вставити його сюди за допомогою st.markdown з тегом <img>.
...
if submitted:
    ...
    if optimization_goal == 'Мінімізація бюджету':
        ...
        if reach_target_pct <= 0:
            ...
            for _, row in df.iterrows():
                # Обмеження MinShare: budget_i >= row['MinShare'] * total_budget_lp_var, f"Min Share {row['Instrument']}"
                model += budget_vars[row['Instrument']] >= row['MinShare'] * total_budget_lp_var, f"Min Share {row['Instrument']}"
                # Обмеження MaxShare: budget_i <= row['MaxShare'] * total_budget_lp_var, f"Max Share {row['Instrument']}"
